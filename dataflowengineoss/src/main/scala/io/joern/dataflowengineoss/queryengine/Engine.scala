...
object Engine {
  ...
  def methodsForCall(call: Call): List[Method] = {
    // Prefer the pointer-analysis-backed resolver if the pass has been run on this CPG. It returns only the precise
    // targets for calls where pointer analysis has info, and transparently falls back to existing CALL-edge targets
    // otherwise — so recall is never worse than the default [[NoResolve]] behaviour, but precision on virtual
    // dispatch (visitor/builder/polymorphic collections) can be substantially better.
    val resolver = PointerAnalysis.resolverFor(call.graph).getOrElse(NoResolve)
    resolver.getCalledMethods(call).toList
  }
