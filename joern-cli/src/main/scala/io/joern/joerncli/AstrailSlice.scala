...
import io.joern.joerncli.AstrailParse.ParserConfig
...
object AstrailSlice {
  ...
  private val configParser = new scopt.OptionParser[BaseConfig[?]]("astrail-slice") {
  ...
  def main(args: Array[String]): Unit = {
    parseConfig(args).foreach { config =>
      if (config.isInstanceOf[DefaultSliceConfig]) {
        ...
        if (needsTempCpg) {
          FileUtil.usingTemporaryFile("astrail-slice", ".bin") { tmpFile =>
