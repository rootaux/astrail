...
private[declarations] trait AstForMethodsCreator { this: AstCreator =>
  ...
  def calcParameterTypes(
    ...
    val parameterTypes = parameters.map { param =>
      tryWithSafeStackOverflow(param.getType).toOption
        ...
        // Fallback to Object so one unresolved param doesn't collapse the entire signature.
  ...
  def composeSignature(
    ...
    (maybeReturnType, maybeParameterTypes) match {
      ...
      // Use ANY return type rather than discarding resolved parameter types.
      case (None, Some(parameterTypes)) =>
        composeMethodLikeSignature(TypeConstants.Any, parameterTypes)
