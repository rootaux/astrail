...
class TypeInferencePass(cpg: Cpg) extends ForkJoinParallelCpgPass[Call](cpg) {
  ...
  private def getReplacementMethod(call: Call): Option[Method] = {
    ...
    cache.get(callKey).toScala.getOrElse {
      ...
      val result = resolvedMethodIndex.get(call.name).flatMap { candidateMethods =>
        val matchingMethods = candidateMethods.filter(isMatchingMethod(_, call, callNameParts)).toList
        matchingMethods match {
          case single :: Nil => Some(single)
          case multiple if multiple.nonEmpty =>
            // Pick the candidate with the most specific (non-ANY) argument type matches.
            val ranked = multiple.sortBy { method =>
              val callArgs = call.argument.toList
              val args     = if (method.modifier.modifierType.iterator.contains(ModifierTypes.STATIC)) callArgs.tail else callArgs
              -method.parameter.zip(args).count { case (param, arg) =>
                val argType = arg.propertyOption(Properties.TypeFullName).getOrElse(TypeConstants.Any)
                argType != TypeConstants.Any && argType == param.typeFullName
              }
            }
            ranked.headOption
          case _ => None
