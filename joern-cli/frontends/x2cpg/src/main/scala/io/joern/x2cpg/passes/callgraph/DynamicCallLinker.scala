...
class DynamicCallLinker(cpg: Cpg) extends CpgPass(cpg) {
  ...
  private def linkUnresolvedSignatureCall(call: Call, dstGraph: DiffGraphBuilder): Boolean = {
    ...
    if (sig == null || !sig.contains("<unresolvedSignature>")) return false

    // Parse `TypeFullName.methodName:<unresolvedSignature>(N)`.
    val mfn      = call.methodFullName
    val colonIdx = mfn.lastIndexOf(':')
    if (colonIdx <= 0) return true
    val typeAndName = mfn.substring(0, colonIdx)
    val lastDot     = typeAndName.lastIndexOf('.')
    if (lastDot <= 0) return true
    val declaringType = typeAndName.substring(0, lastDot)
    val methodName    = typeAndName.substring(lastDot + 1)

    // Parameter count from the `(N)` suffix — this excludes `this` for instance methods, matching Joern
    ...
    // Prefer concrete (non-stub, non-unresolved) candidates; treat the unresolved-stub as a fallback only.
    val concrete = candidates.filterNot(_.signature.contains("<unresolvedSignature>"))
    val tgts     = if (concrete.nonEmpty) concrete else candidates
    if (tgts.isEmpty) return true
