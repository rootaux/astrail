...
trait BridgeBase extends InteractiveShell with ScriptExecution with PluginHandling with ServerHandling {
  ...
  protected def parseConfig(args: Array[String]): Config = {
    val parser = new scopt.OptionParser[Config](applicationName) {
      ...
      opt[Unit]("server-quiet")
        .action((_, c) => c.copy(serverQuiet = true))
        .text(
          "suppress per-query echoing on the server's stdout. The submitted query and its predef are normally " +
            "printed to help debugging; pass this flag when driving the server from a programmatic client " +
            "(e.g. a Python SDK) where the noise interferes with parsing the response stream."
        )
...
trait ServerHandling { this: BridgeBase =>
  ...
  protected def startHttpServer(config: Config): Unit = {
    val baseConfig = replpp.Config(
      ...
      runAfter = buildRunAfterCode(config),
      // Default-on for interactive debugging — pass --server-quiet when driving the server from a programmatic
      // client (e.g. a Python SDK) where echoed queries on stdout interfere with parsing the response stream.
      verbose = !config.serverQuiet,
      classpathConfig = replpp.Config
        .ForClasspath(inheritClasspath = true, dependencies = config.dependencies, resolvers = config.resolvers)
    )
