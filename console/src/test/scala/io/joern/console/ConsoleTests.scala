...
class ConsoleTests extends AnyWordSpec with Matchers {
  ...
  "importCode" should {
    ...
    "provide overview of available language modules" in ConsoleFixture() { (console, _) =>
      console.importCode.toString should include("java")
    }

    "allow importing code with specific module (javasrc2cpg)" in ConsoleFixture() { (console, codeDir) =>
      console.importCode.java(codeDir.toString)
    ...
    "set project to active" in ConsoleFixture() { (console, codeDir) =>
      console.importCode(codeDir.toString, "foo")
      console.cpg.method.name.toSet should not be empty
    }
